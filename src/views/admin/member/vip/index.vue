        ...
        <el-row>
            ...
            <el-col :span="20" :xs="24">
                ...
                <el-table v-loading="loading" :data="vipCardList" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column v-if="columns[5].visible" label="更新时间" align="center" prop="update_datetime"
                        width="160">
                        <template slot-scope="scope">
                            <span>{{ parseTime(scope.row.update_datetime) }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column v-if="columns[5].visible" label="创建时间" align="center" prop="create_datetime"
                        width="160">
                        <template slot-scope="scope">
                            <span>{{ parseTime(scope.row.create_datetime) }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column v-if="hasPermi(['member:vip:post', 'member:vip:post'])" label="操作"
        ...
        <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="100px">
                ...
                <el-row>
                    ...
                    <el-col :span="12">
                        <el-form-item label="优惠价格" prop="discountAmount">
                            <el-input type="number" v-model="form.discountAmount" placeholder="优惠价格" maxlength="11" />
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
...
export default {
    ...
    data() {
        return {
            ...
            columns: [
                ...
                { key: 1, label: `会员卡名称`, visible: true },
                { key: 2, label: `原价(U)`, visible: true },
                { key: 3, label: `优惠价(U)`, visible: true },
    ...
    methods: {
        ...
        getList() {
            this.loading = true;
            listVipCard(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
                this.vipCardList = response.data.results;
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id !== undefined) {
                        ...
                        switch (true) {
                            case this.form.name === '月卡':
                                this.form.vipCardType = 0;
                                break;
                            case this.form.name === '季卡':
                                this.form.vipCardType = 1;
                                break;
                            case this.form.name === '半年卡':
                                this.form.vipCardType = 2;
                                break;
                            case this.form.name === '年卡':
                                this.form.vipCardType = 3;
                                break;
                        }
